...
typedef struct {
    void           *data;
    RcControlBlock *block;
} Rc;

int    rc_init(Rc *self, void *data, DropFn drop_fn);
void   rc_copy(Rc *dst, const Rc *src);
void   rc_move(Rc *dst, Rc *src);
void   rc_drop(Rc *self, DropFn drop_fn);
void  *rc_ptr(const Rc *self);
int    rc_cmp(const Rc *a, const Rc *b);
bool   rc_eq(const Rc *a, const Rc *b);
UInt64 rc_hash(const Rc *self);

/* ==================== Generic PQueue ====================*//*
                                                              * Priority Queue (binary heap).
                                                              * Default behavior is a MAX-HEAP: when using T##_cmp
                                                              * (which returns positive if a > b), the largest element
                                                              * is at the top. For a MIN-HEAP, pass a comparator that
                                                              * returns the negated result of T##_cmp.
                                                              */
...
void rbtmap_insert_cp(RBTMap *self,
        ...
        DropFn                key_drop,
        DropFn                value_drop);
void rbtmap_insert_mv(
        RBTMap *self, void *key, void *value, MoveFn key_move, MoveFn value_move, DropFn key_drop, DropFn value_drop);
...
#define CBASE_RC(SELF, T) \
    typedef struct { \
        Rc impl; \
    } SELF; \
\
    static inline int SELF##_init(SELF *self, T *data) { \
        return rc_init(&self->impl, data, (DropFn)T##_drop); \
    } \
    static inline void SELF##_copy(SELF *dst, const SELF *src) { \
        rc_copy(&dst->impl, &src->impl); \
    } \
    static inline void SELF##_move(SELF *dst, SELF *src) { \
        rc_move(&dst->impl, &src->impl); \
    } \
    static inline void SELF##_drop(SELF *self) { \
        rc_drop(&self->impl, (DropFn)T##_drop); \
    } \
    static inline T *SELF##_ptr(const SELF *self) { \
        return (T *)rc_ptr(&self->impl); \
    } \
    static inline int SELF##_cmp(const SELF *a, const SELF *b) { \
        return rc_cmp(&a->impl, &b->impl); \
    } \
    static inline bool SELF##_eq(const SELF *a, const SELF *b) { \
        return rc_eq(&a->impl, &b->impl); \
    } \
    static inline UInt64 SELF##_hash(const SELF *self) { \
        return rc_hash(&self->impl); \
    } \
    static inline UInt64 SELF##_refcount(const SELF *self) { \
        if (!self->impl.block) \
            return 0; \
        return (UInt64)atomic_load(&self->impl.block->refcount); \
    }

/* ==================== CBASE_PRIORITY_QUEUE Macro ====================*/

/*
 * Generates a type-safe priority queue
 * wrapper. Uses T##_cmp by default (MAX-HEAP).
 * Use SELF##_init_with_cmp for a custom
 * comparator (e.g. pass a negated cmp for
 * MIN-HEAP).
 */
...
#define CBASE_RBTMAP(SELF, K, V) \
    ...
    static inline int SELF##_init(SELF *self) { \
        ...
        return rbtmap_init(&self->impl, sizeof(K), sizeof(V), offsetof(SELF##_Entry, _value), (CmpFn)K##_cmp); \
    } \
    static inline int SELF##_init_with_cmp(SELF *self, CmpFn cmp_fn) { \
        typedef struct { \
            K _key; \
        ...
        return rbtmap_init(&self->impl, sizeof(K), sizeof(V), offsetof(SELF##_Entry, _value), cmp_fn); \
    } \
    static inline void SELF##_drop(SELF *self) { \
    ...
    static inline void SELF##_copy(SELF *dst, const SELF *src) { \
        rbtmap_copy(&dst->impl, &src->impl, (CopyFn)K##_copy, (CopyFn)V##_copy, (DropFn)K##_drop, (DropFn)V##_drop); \
    } \
    static inline void SELF##_move(SELF *dst, SELF *src) { \
...
int rc_init(Rc *self, void *data, DropFn drop_fn) {
    RcControlBlock *block = xrealloc(NULL, sizeof(RcControlBlock));
    if (!block)
        return -1;
    atomic_init(&block->refcount, 1);
    block->drop_fn = drop_fn;
    self->data     = data;
    self->block    = block;
    return 0;
}
...
void rc_copy(Rc *dst, const Rc *src) {
    if (src->block) {
        atomic_fetch_add_explicit(&src->block->refcount, 1, memory_order_relaxed);
    }
    dst->data  = src->data;
    dst->block = src->block;
}
...
void rc_drop(Rc *self, DropFn drop_fn) {
    if (!self->block)
        return;
    RcControlBlock *block = self->block;
    unsigned        prev  = atomic_fetch_sub_explicit(&block->refcount, 1, memory_order_acq_rel);
    if (prev == 1) {
        if (drop_fn && self->data) {
            drop_fn(self->data);
        }
        free(block);
    }
...
bool rc_eq(const Rc *a, const Rc *b) {
    return rc_ptr(a) == rc_ptr(b);
}

UInt64 rc_hash(const Rc *self) {
    void  *ptr = rc_ptr(self);
    UInt64 val = (UInt64)(uintptr_t)ptr;
    return fnv64(&val, sizeof(val), FNV_OFFSET_BASIS);
}
...
static void rbtmap_remove_fixup(RBTMap *self, RBTNode *x, RBTNode *xparent) {
    while (x != self->root && (x == NULL || x->color == RB_BLACK)) {
        if (x == xparent->left) {
            ...
            if ((w->left == NULL || w->left->color == RB_BLACK) && (w->right == NULL || w->right->color == RB_BLACK)) {
                w->color = RB_RED;
                x        = xparent;
                xparent  = x->parent;
            } else {
            ...
            if ((w->right == NULL || w->right->color == RB_BLACK) && (w->left == NULL || w->left->color == RB_BLACK)) {
                w->color = RB_RED;
                x        = xparent;
                xparent  = x->parent;
            } else {
...
static void rbtmap_copy_subtree(RBTMap *dst, RBTNode *src_node, CopyFn key_copy, CopyFn value_copy) {
    if (src_node == NULL)
        return;
    rbtmap_copy_subtree(dst, src_node->left, key_copy, value_copy);
    rbtmap_insert_cp(
            dst, rbtmap_node_key(src_node), rbtmap_node_value(dst, src_node), key_copy, value_copy, NULL, NULL);
...
/* ==================== Quadruplet struct and traits ==================== */

typedef struct {
    Int32 a, b, c, d;
} Quadruplet;

/* Forward declarations of trait functions (needed before CBASE_VECTOR) */
void   Quadruplet_drop(Quadruplet *self);
int    Quadruplet_cmp(Quadruplet *a, Quadruplet *b);
bool   Quadruplet_eq(Quadruplet *a, Quadruplet *b);
UInt64 Quadruplet_hash(Quadruplet *self);
void   Quadruplet_copy(Quadruplet *dst, const Quadruplet *src);
void   Quadruplet_move(Quadruplet *dst, Quadruplet *src);

CBASE_VECTOR(QuadrupletVec, Quadruplet)

/* ==================== Quadruplet trait implementations ==================== */

void Quadruplet_drop(Quadruplet *self) { (void)self; }
...
UInt64 Quadruplet_hash(Quadruplet *self) {
    return fnv64(self, sizeof(Quadruplet), FNV_OFFSET_BASIS);
}

void Quadruplet_copy(Quadruplet *dst, const Quadruplet *src) { *dst = *src; }

void Quadruplet_move(Quadruplet *dst, Quadruplet *src) { *dst = *src; }

/* ==================== Helper: step forward (skip duplicates) ==================== */

static Int32 step_forward(int *nums, Int32 len, Int32 i) {
    Int32 cur = nums[i];
...
/* ==================== Helper: step backward (skip duplicates) ==================== */

static Int32 step_backward(int *nums, Int32 i) {
    Int32 cur = nums[i];
...
int** fourSum(int* nums, int numsSize, int target, int* returnSize, int** returnColumnSizes) {
    /* Use QuadrupletVec to collect results */
    QuadrupletVec result_vec;
    QuadrupletVec_init(&result_vec);
    QuadrupletVec_reserve(&result_vec, 16);

    /* Sort nums directly */
    qsort(nums, (size_t)numsSize, sizeof(int), (CmpFn)Int32_cmp);

    Int32 len = (Int32)numsSize;
    Int32 i = 0;
    Int32 j, k, l;
    ...
    while (i < len - 3) {
        ...
        while (j < len - 2) {
            ...
            l = len - 1;
            while (k < l) {
                Int64 sum = (Int64)nums[i] + (Int64)nums[j] + (Int64)nums[k] + (Int64)nums[l];
                if (sum > target) {
                    l = step_backward(nums, l);
                } else if (sum < target) {
                    k = step_forward(nums, len, k);
                } else {
                    /* Found a quadruplet */
                    Quadruplet q;
                    q.a = nums[i];
                    ...
                    q.c = nums[k];
                    q.d = nums[l];
                    QuadrupletVec_pushcp(&result_vec, &q);
                    k = step_forward(nums, len, k);
                    l = step_backward(nums, l);
                }
            }
            j = step_forward(nums, len, j);
        }
        i = step_forward(nums, len, i);
    }

    /* Convert QuadrupletVec to int** */
    size_t count = QuadrupletVec_len(&result_vec);
    *returnSize = (int)count;
    if (count == 0) {
        QuadrupletVec_drop(&result_vec);
    ...
    *returnColumnSizes = malloc(sizeof(int) * count);
    for (size_t idx = 0; idx < count; idx++) {
        Quadruplet *qp = QuadrupletVec_get(&result_vec, idx);
        result[idx] = malloc(sizeof(int) * 4);
        result[idx][0] = (int)qp->a;
        result[idx][1] = (int)qp->b;
        result[idx][2] = (int)qp->c;
        result[idx][3] = (int)qp->d;
        (*returnColumnSizes)[idx] = 4;
    }
