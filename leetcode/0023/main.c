...
/* ==================== ListNodePtr Type and Traits ==================== */
typedef struct ListNode ListNode;
typedef ListNode *ListNodePtr;

void ListNodePtr_drop(ListNodePtr *self) {
    (void)self;
    /* no-op: we don't own the nodes, just rearranging pointers */
}

int ListNodePtr_cmp(ListNodePtr *a, ListNodePtr *b) {
    if ((*a)->val < (*b)->val)
        return -1;
    if ((*a)->val > (*b)->val)
        return 1;
...
bool ListNodePtr_eq(ListNodePtr *a, ListNodePtr *b) {
    return *a == *b;
}

UInt64 ListNodePtr_hash(ListNodePtr *self) {
    return fnv64(self, sizeof(ListNodePtr), FNV_OFFSET_BASIS);
}

void ListNodePtr_copy(ListNodePtr *dst, const ListNodePtr *src) {
    *dst = *src;
}

void ListNodePtr_move(ListNodePtr *dst, ListNodePtr *src) {
    *dst = *src;
}

void ListNodePtr_swap(ListNodePtr *a, ListNodePtr *b) {
    ListNodePtr tmp = *a;
    *a              = *b;
    *b              = tmp;
}

CBASE_PRIORITY_QUEUE(ListNodePtrPQueue, ListNodePtr)

/* ==================== Solution ==================== */

/* Reverse comparator for min-heap */
static int listnodeptr_cmp_reverse(ListNodePtr *a, ListNodePtr *b) {
    return -ListNodePtr_cmp(a, b);
}
...
struct ListNode *mergeKLists(struct ListNode **lists, int listsSize) {
    ...
    ListNodePtrPQueue pq;
    ListNodePtrPQueue_init_with_cmp(&pq, (CmpFn)listnodeptr_cmp_reverse);

    /* Push the first node of each non-empty list into the min-heap */
    for (int i = 0; i < listsSize; i++) {
        if (lists[i] != NULL) {
            ListNodePtr ptr = lists[i];
            ListNodePtrPQueue_pushcp(&pq, &ptr);
        }
    }

    /* Dummy head for building the result list */
    ListNode dummy = {0, NULL};
    ListNode *tail = &dummy;

    while (!ListNodePtrPQueue_is_empty(&pq)) {
        ListNodePtr *top  = ListNodePtrPQueue_top(&pq);
        ListNode    *node = *top;
        ListNodePtrPQueue_pop(&pq);

        tail->next = node;
        tail       = node;

        if (node->next != NULL) {
            ListNodePtr next = node->next;
            ListNodePtrPQueue_pushcp(&pq, &next);
        }
    }

    tail->next = NULL;
    ListNodePtrPQueue_drop(&pq);
